...
class DeterministicActorCriticNet(nn.Module):
    def __init__(self, state_dim, action_dim, obs_state_range,  action_range, num_atoms,
        ...
        self.v_min_critic = 0.
        self.v_max_critic = 20.
        self.v_min_actor = -1.
        self.v_max_actor = 1.
        self.num_atoms = num_atoms

        # init output layers
        self.fc_action = layer_init(nn.Linear(actor_body.feature_dim, action_dim).to(device), 1e-3).to(self.device)
        self.fc_critic = layer_init(nn.Linear(critic_body.feature_dim, num_atoms).to(device), 1e-3).to(self.device)
    ...
    def action(self, phi):
        ...
        return torch.tensor(0.5).double().to(self.device) * (torch.tensor(self.v_max_actor + self.v_min_actor).double()+torch.tensor((self.v_max_actor - self.v_min_actor)).double()*torch.tanh(self.fc_action(self.actor_body.forward(phi)))).to(self.device)
   
    def get_probs(self, data_phi, data_action, data_q):
        return torch.nn.functional.softmax(self.fc_critic(self.critic_body.forward(data_phi, data_action, data_q)), dim = 1)#*self.fc_critic(self.critic_body.forward(data_phi, data_action, data_q))
    
    # prediction for critic's neural network
    def critic(self, data_phi, data_action, data_q):
        
        probs = torch.tensor(0.5).double() * (torch.tensor(self.v_max_critic + self.v_min_critic).double()+torch.tensor((self.v_max_critic - self.v_min_critic)).double()*torch.tanh(self.fc_critic(self.critic_body.forward(data_phi, data_action, data_q)))).to(self.device)
